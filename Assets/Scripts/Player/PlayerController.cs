...
public class PlayerController : MonoBehaviour
{
    ...
    private void Awake() {
        ...
        for (int i = 0; i < m_Attacks.Length; i++) {
            ...
            if (attack.WindUpTime > attack.FrozenTime) {
                Debug.LogError(attack.AttackName + " has a wind up time that is larger than the amount the time that the player is frozen for");
            }
        }
    }
    ...
    private void Update() {
        ...
        // set how hard player presses movement buttons
        ...
        if (Mathf.Abs(forward) < moveThreshold) {
            forward = 0;
        }
        if (Mathf.Abs(right) < moveThreshold) {
            right = 0;
        }
